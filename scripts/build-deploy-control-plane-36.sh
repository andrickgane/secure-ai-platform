...
perl -0pi -e '
s#registry\.andrick\.local:31039/ai-platform/control-plane:v2\.1\.0-dev\.[0-9]+#registry.andrick.local:31039/ai-platform/control-plane:v2.1.0-dev.36#g
' deploy/api/deployment.yaml


COUNT="$(
  grep -c \
    'registry.andrick.local:31039/ai-platform/control-plane:v2.1.0-dev.36' \
    deploy/api/deployment.yaml
)"

if [[ "${COUNT}" != "2" ]]; then
    echo "ERROR: expected exactly 2 .36 references"
    echo "Found: ${COUNT}"
    exit 1
fi
...
IMAGE_COUNT="$(
  echo "${RUNNING_IMAGES}" \
  | grep -c \
    'control-plane:v2.1.0-dev.36'
)"

if [[ "${IMAGE_COUNT}" != "2" ]]; then
    echo
    echo "ERROR: .36 not used by both init + API"
    exit 1
fi
